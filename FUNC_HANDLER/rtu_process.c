...
void RTU_Send_Cmd(u8 index)
{
	switch(rtu_structure.cmd[index])
	{
		...
		case RTU_AC_EX_ON:
		case RTU_AC_EX_OFF:
			rtu_protocol_format_wr(ADDR_SET_AC_RELAY,1,(u16 *)&rtu_structure.cmd_data[index]);
			break;
		case RTU_DC2_ON:
		case RTU_DC2_OFF:
		...
		case RTU_DC3_ON:
		case RTU_DC3_STOP:
			rtu_protocol_format_wr(ADDR_SET_DC3,1,(u16 *)&rtu_structure.cmd_data[index]);
			break;
		case RTU_AC1_ON:
			rtu_protocol_format_wr(ADDR_SET_AC1,1,(u16 *)&rtu_structure.cmd_data[index]);
			break;		
		case RTU_MOTOR_FLAG:
			rtu_protocol_format_wr(ADDR_MOTOR_FLAG,1,(u16 *)&rtu_structure.cmd_data[index]);
			break;						
		case RTU_IDlE:
			rtu_protocol_format_rd(ADDR_DOOR,ADDR_HREG_TOTAL);
			break;	
		default:
			break;
	}
